...
# STREAMLIT UI LAYOUT
# -----------------------------------------------------------------------------------------
st.set_page_config(page_title="Florida Real Estate Deadline Calculator v7", layout="wide")

st.title("🌴 Florida Real Estate Contract Deadline Calculator v7.0")
st.markdown("""
An advanced comparative deadline calculator featuring all core milestones, **Cash vs. Financing toggles**, **AS-IS vs. Standard Inspection toggles**, **Buyer's Additional Deposit**, **condo resale rescission periods**, and **dispute-specific inspection timelines** under **NABOR** and **FAR/BAR** rules. This dashboard is perfectly synchronized with the Excel v7.0 Tracker.
""")
...
# Options and Toggles for v6.0 / v7.0
...
enable_dispute = has_inspection == "Yes" and contract_style == "Standard"
election_date = None
...
with tab2:
    ...
    # -----------------------------------------------------------------------------------------
    # NEW: DYNAMIC PLAIN TEXT CLIENT TIMELINES (REPRESENTING THE EXACT FORMAT REQUESTED)
    # -----------------------------------------------------------------------------------------
    st.markdown("---")
    st.subheader("📋 Plain Text Client Timelines (Side-by-Side Summary)")
    st.markdown("""
...
# -----------------------------------------------------------------------------------------
summary_text = f"""FLORIDA REAL ESTATE CONTRACT DEADLINE CHRONOLOGICAL SUMMARY (V7.0)
Effective Date: {eff_date}
Scheduled Closing Date: {closing_date}
Funding Type: {funding_type}
Inspection Option: {has_inspection} (Style: {contract_style if has_inspection == 'Yes' else 'N/A'})
...
st.sidebar.download_button(
    ...
    data=summary_text,
    file_name="florida-contract-deadlines-schedule-v7.txt",
    mime="text/plain"
)
