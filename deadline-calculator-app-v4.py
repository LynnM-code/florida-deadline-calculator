...
# STREAMLIT UI LAYOUT
# -----------------------------------------------------------------------------------------
st.set_page_config(page_title="Florida Real Estate Deadline Calculator v4", layout="wide")

st.title("🌴 Florida Real Estate Contract Deadline Calculator v4.0")
st.markdown("""
...
# -----------------------------------------------------------------------------------------
# DISPLAY SIDE-BY-SIDE CALCULATOR
# -----------------------------------------------------------------------------------------
...
summary_text = f"""FLORIDA REAL ESTATE CONTRACT DEADLINE SUMMARY (V4.0)
Effective Date: {eff_date}
Scheduled Closing Date: {closing_date}

NABOR CONTRACT TIMELINES:
- Initial Escrow Deposit: {nab_dep_date} ({dep_offset} days after)
- Buyer's Additional Deposit: {nab_add_dep_date} ({add_dep_offset} days after)
- Buyer's Loan Application: {nab_loan_app} ({loan_app_offset} days after)
- Inspection Period: {nab_insp_date} ({insp_offset} days after)
- Buyer Defective Items Election: {nab_election} ({election_offset} days after Inspection Period)
"""

if enable_dispute:
    summary_text += f"- Seller Response to Election: {nab_seller_resp} ({seller_offset} days after Buyer Election)\n- Buyer Right to Terminate: {nab_terminate} ({terminate_offset} days after Seller Response)\n"
if enable_assoc:
    summary_text += f"- Buyer Application for HOA Approval: {nab_assoc} ({assoc_app_offset} days after receipt)\n"
if enable_condo:
    summary_text += f"- Condo Document Rescission: {nab_condo} ({condo_resciss_offset} Business Days after receipt)\n"

summary_text += f"""- Financing Contingency: {nab_fin} ({nab_fin_offset} days after)
- Title Evidence: {nab_title} ({title_offset} days prior)
- Survey Deadline: {nab_survey} ({survey_offset} days prior)
- Actual Closing Date: {rolled_closing_nab}
"""

st.sidebar.download_button(
    label="📥 Download Schedule Summary",
    data=summary_text,
    file_name="florida-contract-deadlines-schedule-v4.txt",
    mime="text/plain"
)
